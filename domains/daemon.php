<?php
	...
	class DomainsDaemon {
		...
		protected function createVirtualHost($domain, $path) {
			...
			$config .= sprintf('	ErrorLog %s%s_error.log', $logs, $domain->name) . PHP_EOL;
			$config .= sprintf('	CustomLog %s%s_access.log combined', $logs, $domain->name) . PHP_EOL;
				
			$config .= PHP_EOL;
			#$config .= sprintf('		Alias /errors/403.html /etc/fruithost/placeholder/errors/403.html', $path) . PHP_EOL;
			#$config .= sprintf('		Alias /errors/404.html /etc/fruithost/placeholder/errors/404.html', $path) . PHP_EOL;
			...
			$config .= PHP_EOL;
			$config .= '	<Files ~ "(^\.|php\.ini$)">' . PHP_EOL;
			$config .= '		Require all denied' . PHP_EOL;
			$config .= '	</Files>' . PHP_EOL;
